...
def check_ru_sub_keys(keys: list[str], n: int):
    """
    Проверяет уникальность сокращенных ключей фиксированной длины.

    Для каждого исходного ключа извлекаются последние `n` символов.
    Если среди полученных значений обнаруживается коллизия,
    функция увеличивает длину суффикса на единицу и выполняет
    проверку повторно.

    Процесс продолжается рекурсивно до тех пор, пока все
    сокращенные ключи не станут уникальными.

    Args:
        keys (list[str]): Список исходных ключей для проверки.
        n (int): Количество символов, извлекаемых с конца ключа.

    Returns:
        None

    Side Effects:
        - Выводит информацию о найденных коллизиях в консоль.
        - При отсутствии коллизий выводит минимальную длину
          уникального суффикса.
        - Выполняет рекурсивные вызовы самой себя.

    Notes:
        Алгоритм используется для определения минимального количества
        последних символов, достаточного для уникальной идентификации
        всех ключей.

    Examples:
        >>> check_ru_sub_keys(
        ...     ["abc123456", "def123456", "xyz654321"],
        ...     3
        ... )
        Error: ...
        n = 7
